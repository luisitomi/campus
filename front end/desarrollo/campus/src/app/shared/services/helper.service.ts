...
export class HelperService {
  ...
  public static findHttpResponseMessage(messageToFind: string, data: HttpResponse<any> | any, seachInCaptionOnly = true, includeCaptionInResult = false): string {
    ...
    for (let message of httpMessages) {
      let fullMessage = HelperService.splitInTwo(message, this.captionAndMessageSeparator);
  ...
  public static getQueryParamsFromString(paramString: string) {
    ...
    for (let param of paramString.split("&")) {
      let keyValue = HelperService.splitInTwo(param, "=");
  ...
  public static printDate(date: Date, separator = "at") {
    return `${HelperService.printDateOnly(date)} ${separator} ${HelperService.printTimeOnly(date)}`;
  }
  ...
  public static printFriendlyDate(date: Date, separator = "-") {
    ...
    if (test.toDateString() == today.toDateString())
      return `Today ${separator} ${HelperService.printTimeOnly(date)}`;
    if (test.toDateString() == yesterday.toDateString())
      return `Yesterday ${separator} ${HelperService.printTimeOnly(date)}`;
    else
  ...
  public static printShortDate(date: Date, separator = "/", dateTimeSeparator = "-") {
    ...
    return `${month}${separator}${day}${separator}${year} ${dateTimeSeparator} ${HelperService.printTimeOnly(date)}`;
  }
  ...
  public static convertToAbsoluteUrl(url: string) {
    ...
    return HelperService.testIsAbsoluteUrl(url) ? url : '//' + url;
  }
  ...
  public static removeNulls(obj) {
    ...
    for (let k in obj) {
      if (obj[k] === null) {
        ...
        HelperService.removeNulls(obj[k]);
      }

      if (isArray && obj.length == k) {
        HelperService.removeNulls(obj);
      }
    }
  ...
  public static getPageInfo(page, info) {
    if (info != null) {
      if (info.pageSize != undefined) {
        page.pageNumber = info.offset;
      }
      else {
        if (info.sorts != undefined && info.sorts.length >= 1) {
          page.property = info.sorts[0].prop;
          page.dir = info.sorts[0].dir;
          page.pageNumber = AppConstants.Paginado.DEFAULT_PAGE_INDEX_PAGINATION;
        }
      }
    }
    else {
