...
@Component({
  selector: 'multitable-update-modal',
  templateUrl: './multitable-update-modal.component.html'
})

export class ModalMultiTableUpdateComponent implements OnInit {
  ...
  byid(){
    const searchData = [];
      for (const l of this.multi) {
        if (l.itemId.toString().toLowerCase().indexOf(this.idInt.toString().toLowerCase()) > -1) {
          searchData.push(l)
        }
      }
      if (searchData.length > 0) {
       this.des = searchData[0].second
